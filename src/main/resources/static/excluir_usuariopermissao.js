...
document.querySelectorAll('.excluir').forEach(function(button) {
    ...
    function() {
        if (confirm('Confirma a exclusão?')) {
            ...
            const permissaoId = this.dataset.permissaoId;
            const usuarioIdExcluirPermissao = this.dataset.usuarioIdExcluirPermissao;

            
            // Realize a chamada AJAX para excluir o recurso
            fetch(`/listapermissoesusuario/${permissaoId}`, {
                method: 'DELETE',
                headers: {
